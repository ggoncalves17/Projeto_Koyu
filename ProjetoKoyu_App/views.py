...
from .models import Categoria, Equipamento, Exercicios, Historico, PlanoTreinos, Utilizador, UtilizadorManager
...
# Está a adicionar várias categorias e apenas 1 equipamento
# Não devia de ser ao contrário?
# Fui em conta com o que estava no models
def AdicionarExercicioView(request):
    if request.method == 'POST':
        nome = request.POST.get("nomeExercicio")
        descricao = request.POST.get("descricaoExercicio")
        video = request.FILES.get("videoExercicio")
        foto = request.FILES.get("fotoCapaFile") 
        intensidade = request.POST.get("intensidade")
        categorias_ids = request.POST.get("categoriasSelecionada")
        equipamento_id = int(request.POST.get("equipamentoSelecionado"))

        print(request, video)
        
        # Verifica se o exercicio já existe
        if Exercicios.objects.filter(ex_nome=nome).exists():
            messages.error(request, "Exercício já existe!")
        else:
            # Criação do exercício
            exercicio = Exercicios.objects.create(
                ex_nome=nome, 
                ex_descricao=descricao, 
                ex_intensidade=intensidade, 
                ex_foto=foto, 
                ex_video=video, 
    ...
    equipamentos = Equipamento.objects.all()  # Obter todos os equipamentos
    categorias = Categoria.objects.all()  # Obter todas as categorias
    
    return render(request, 'projeto_koyu/adicionarExercicio-form.html', {'equipamentos': equipamentos, 'categorias': categorias})

    
def EditarExercicioView(request):
    return render(request, 'projeto_koyu/editarExercicio-form.html')
    
def DetalhesExercicioView(request, exercicio_id):
    exercicio = get_object_or_404(Exercicios, id=exercicio_id)
    return render(request, 'projeto_koyu/detalhesExercicio-form.html', {'exercicio': exercicio})
    
def AdicionarTreinoView(request):
    return render(request, 'projeto_koyu/adicionarTreino-form.html')
     
def EditarTreinoView(request):
    return render(request, 'projeto_koyu/editarTreino-form.html')

def DetalhesTreinoView(request, treino_id):
    treino = get_object_or_404(PlanoTreinos, id=treino_id)

    # Obtém os exercícios associados ao treino
    exercicios_selecionados = Exercicios.objects.filter(series_planostreino__planotreinos=treino)

    # Renderiza o template com os dados do treino e dos exercícios selecionados
    return render(request, 'projeto_koyu/detalhesTreino-form.html', {
        'treino': treino,
